...
class CallModeConfig(Base):
    ...
    mode_a_enabled: Mapped[bool] = mapped_column(
        Boolean, nullable=False, server_default=text("true")
    )
    mode_b_enabled: Mapped[bool] = mapped_column(
        Boolean, nullable=False, server_default=text("false")
    )
    access_control: Mapped[str] = mapped_column(
        String(20), nullable=False, server_default="everyone"
    )
    ...
    user: Mapped["User | None"] = relationship(
        "User", back_populates="call_mode_config", uselist=False
    )

...
class HandoffOffer(Base):
    ...
    selected_device_id: Mapped[uuid.UUID | None] = mapped_column(
    ...
    __table_args__ = (
        UniqueConstraint("call_id", name="uq_handoff_offers_call"),
        CheckConstraint(
            "status IN ('offered', 'accepted', 'declined', 'expired')",
            name="ck_handoff_offers_status",
        ),
        CheckConstraint(
            "offer_reason IS NULL OR offer_reason IN ('always', 'vip', 'urgent', 'vip_and_urgent')",
            name="ck_handoff_offers_reason",
        ),
        Index("handoff_offers_user_idx", "owner_user_id"),
        Index("handoff_offers_pending_idx", "status", "expires_at"),
    )

...
class NotificationDelivery(Base):
    ...
    __table_args__ = (
        CheckConstraint("provider IN ('apns', 'fcm')", name="ck_notification_deliveries_provider"),
        CheckConstraint(
            "status IN ('queued', 'sending', "
            "'provider_accepted', "
            "'provider_rejected', "
            "'delivered_ack', 'expired', "
            "'cancelled')",
            name="ck_notification_deliveries_status",
        ),
        UniqueConstraint(
            "notification_id", "device_id", name="uq_notification_deliveries_notif_device"
        ),
    )

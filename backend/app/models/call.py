...
class Call(Base):
    ...
    direction: Mapped[str] = mapped_column(
        String(10), nullable=False, server_default=text("inbound")
    )
    ...
    voice_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("voice_catalog.id", ondelete="SET NULL")
    )
    forwarding_detected: Mapped[bool] = mapped_column(
        Boolean, nullable=False, server_default=text("false")
    )
    forwarding_verification_hint: Mapped[str | None] = mapped_column(Text)
    missing_summary: Mapped[bool] = mapped_column(
        Boolean, nullable=False, server_default=text("true")
    )
    missing_transcript: Mapped[bool] = mapped_column(
        Boolean, nullable=False, server_default=text("true")
    )
    missing_labels: Mapped[bool] = mapped_column(
        Boolean, nullable=False, server_default=text("true")
    )
    from_masked: Mapped[str] = mapped_column(Text, nullable=False)
    to_masked: Mapped[str] = mapped_column(Text, nullable=False)
    handoff_eligible: Mapped[bool] = mapped_column(
        Boolean, nullable=False, server_default=text("false")
    )
    ...
    handoff_attempt_count: Mapped[int] = mapped_column(
        Integer, nullable=False, server_default=text("0")
    )
    handoff_failure_reason: Mapped[str | None] = mapped_column(Text)
    handoff_loop_detected: Mapped[bool] = mapped_column(
        Boolean, nullable=False, server_default=text("false")
    )
    ...
    __table_args__ = (
        CheckConstraint(
            "source_type IN ('dedicated_number', 'forwarded')", name="ck_calls_source_type"
        ),
        CheckConstraint("direction IN ('inbound')", name="ck_calls_direction"),
        CheckConstraint(
            "status IN ('created', 'inbound_received', "
            "'twiml_responded', 'in_progress', "
            "'completed', 'partial', 'failed', "
            "'canceled')",
            name="ck_calls_status",
        ),
        CheckConstraint("provider IN ('twilio')", name="ck_calls_provider"),
        CheckConstraint(
            "handoff_status IS NULL OR "
            "handoff_status IN ("
            "'not_applicable', "
            "'eligible_pending_minimum_data', "
            "'urgent_candidate', 'offered', "
            "'accepted', 'declined', 'expired', "
            "'failed', 'transfer_starting', "
            "'transfer_connected', "
            "'transfer_failed', "
            "'transfer_cancelled')",
            name="ck_calls_handoff_status",
        ),
        UniqueConstraint("provider", "provider_call_sid", name="uq_calls_provider_sid"),
        Index("calls_user_started_idx", "owner_user_id"),
        Index("calls_user_status_idx", "owner_user_id", "status"),
        Index("calls_provider_sid_idx", "provider_call_sid"),
        Index("calls_user_caller_hash_idx", "owner_user_id", "caller_phone_hash"),

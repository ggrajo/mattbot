...
class ProviderEvent(Base):
    ...
    __table_args__ = (
        CheckConstraint("provider IN ('twilio', 'elevenlabs')", name="ck_provider_events_provider"),
        CheckConstraint(
            "process_status IN ('received', 'processed', 'failed')",
            name="ck_provider_events_status",
        ),
        Index("provider_events_status_idx", "process_status"),
        Index("provider_events_call_sid_idx", "provider_call_sid"),
    )

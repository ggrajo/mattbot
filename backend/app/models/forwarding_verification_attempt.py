...
class ForwardingVerificationAttempt(Base):
    ...
    user: Mapped["User | None"] = relationship("User", back_populates="forwarding_verification_attempts", uselist=False)

    __table_args__ = (CheckConstraint("status IN ('pending', 'passed', 'failed', 'expired')", name="ck_fwd_verification_status"), Index("ix_fwd_verification_user_status", "owner_user_id", "status"),)

...
from sqlalchemy import Boolean, CheckConstraint, DateTime, Float, ForeignKey, Index, Integer, LargeBinary, String, Text, text
...
class CallMemoryItem(Base):
    ...
    source_call_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("calls.id", ondelete="SET NULL"))
    ...
    __table_args__ = (CheckConstraint("memory_type IN ('caller_display_name', 'relationship_tag', 'callback_window_preference', 'communication_preference', 'repeated_reason_pattern')", name="ck_call_memory_items_type"), Index("call_memory_items_user_active_idx", "owner_user_id"), Index("call_memory_items_user_caller_active_idx", "owner_user_id", "caller_phone_hash"),)

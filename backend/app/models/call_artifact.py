...
class CallArtifact(Base):
    ...
    labels_status: Mapped[str] = mapped_column(
        String(20), nullable=False, server_default=text("pending")
    )
    ...
    transcript_provider: Mapped[str] = mapped_column(
        String(20), nullable=False, server_default=text("elevenlabs")
    )
    ...
    __table_args__ = (
        CheckConstraint(
            "summary_status IN ('pending', 'processing', 'ready', 'failed', 'not_available')",
            name="ck_call_artifacts_summary_status",
        ),
        CheckConstraint(
            "labels_status IN ('pending', 'processing', 'ready', 'failed', 'not_available')",
            name="ck_call_artifacts_labels_status",
        ),
        CheckConstraint(
            "transcript_status IN ('pending', 'processing', 'ready', 'failed', 'not_available')",
            name="ck_call_artifacts_transcript_status",
        ),
        Index("call_artifacts_call_id_idx", "call_id"),
        Index("call_artifacts_owner_idx", "owner_user_id"),
    )

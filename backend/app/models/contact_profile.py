...
class ContactProfile(Base):
    ...
    __table_args__ = (
        UniqueConstraint("owner_user_id", "phone_hash", name="uq_contact_profiles_user_phone"),
        Index("contact_profiles_user_idx", "owner_user_id"),
        Index("contact_profiles_user_category_idx", "owner_user_id", "category"),
        CheckConstraint(
            "ai_temperament_preset IS NULL OR "
            "ai_temperament_preset IN ("
        ...
        CheckConstraint(
            "ai_swearing_rule IS NULL OR "
            "ai_swearing_rule IN ("
            "'no_swearing', 'mirror_caller', 'allow')",
            name="ck_contact_profiles_swearing",
        ),
        CheckConstraint(
            "ai_greeting_template IS NULL OR "
            "ai_greeting_template IN ("
